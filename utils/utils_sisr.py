...
def crmin(x,y_real, inplace=False):
    # complex/real minimum
    x0 = x.clone() if not inplace else x
    x0[cabs2(x0) > y_real**2] = torch.tensor([y_real, 0], dtype=x.dtype, device=x.device)
    return x
...
def p2o(psf, shape, psf_center=None):
    '''
    ...
    for axis, axis_size in enumerate(psf.shape[2:]):
        shift = -int(axis_size / 2) if psf_center is None else -psf_center[axis]
        otf = torch.roll(otf, shift, dims=axis+2)
...
def pre_calculate(x, k, sf, k_c=None):
    '''
    Args:
        x: NxCxHxW, LR input
        k: NxCxhxw
    ...
    '''
    w, h = x.shape[-2:]
    FB = p2o(k, (w*sf, h*sf), k_c)
    FBC = cconj(FB, inplace=False)
    F2B = r2c(cabs2(FB))
    #STy = upsample(x, sf=sf)
    #FBFy = cmul(FBC, torch.rfft(STy, 2, onesided=False))
    FBFy = cmul(FBC, torch.rfft(x, 2, onesided=False).repeat(1,1,sf,sf,1))
    return FB, FBC, F2B, FBFy

def pre_calculate_preco(x, k, otf_preco, sf, k_c=None):
    '''
    Args:
        x: NxCxHxW, LR input
        k: NxCxhxw
    ...
    '''
    w, h = x.shape[-2:]
    FB = cdiv( p2o(k, (w*sf, h*sf), k_c), otf_preco)
    FBC = cconj(FB, inplace=False)
    F2B = r2c(cabs2(FB))
    #STy = upsample(x, sf=sf)
    #FBFy = cmul(FBC, torch.rfft(STy, 2, onesided=False))
...
def pre_calculate_thresholded(x, k, thresold, sf):
    '''
    Args:
        x: NxCxHxW, LR input
        k: NxCxhxw
    ...
    '''
    w, h = x.shape[-2:]
    FB = p2o(k, (w*sf, h*sf)) * thresold
    crmin(FB, 1, inplace=True)
    FBC = cconj(FB, inplace=False)
    F2B = r2c(cabs2(FB))
    #STy = upsample(x, sf=sf)
    #FBFy = cmul(FBC, torch.rfft(STy, 2, onesided=False))
...
def pre_calculate_filt(x_shape, k, sf):
    '''
    Args:
        x: NxCxHxW, LR input
        k: NxCxhxw
...
########################################################################################################################
# Re-implementation for true complex datatype
########################################################################################################################


def pre_calculate_cmplx(y, FB, sf):
    '''
    Same as pre_calculate but using true complex datatype and using the otf FB (already in the Fourier domain) instead of the spatial kernel.
    '''
    FBC = torch.conj(FB)
    F2B = torch.real(FB)**2 + torch.imag(FB)**2
    aaT = torch.sum(splits_cmplx(F2B, sf), dim=-1, keepdim=False) / (sf ** 2)
    aaT = aaT.repeat(1, 1, sf, sf)
    FBFy = FBC * torch.fft.fftn(y, dim=(-2, -1)).repeat(1, 1, sf, sf)
    return FBC, aaT, FBFy

#def data_solution_cmplx(x, FB, FBC, F2B, FBFy, rho, sf):
#    FR = FBFy + torch.fft.fftn(rho*x, dim=(-2, -1))
#    x1 = FB * FR
#    FBR = torch.sum(splits_cmplx(x1, sf), dim=-1, keepdim=False) / (sf**2)
#    invW = torch.sum(splits_cmplx(F2B, sf), dim=-1, keepdim=False) / (sf**2)
#    invWBR = FBR / (invW + rho)
#    FCBinvWBR = FBC * invWBR.repeat(1, 1, sf, sf)
#    FX = (FR-FCBinvWBR) / rho
#    Xest = torch.fft.irfftn(FX, FX.shape[-2:], dim=(-2, -1))
#    return Xest

def data_solution_cmplx(x, FB, FBC, aaT, FBFy, rho, sf):
    Fx = torch.fft.fftn(x, dim=(-2, -1))
    ax = torch.sum(splits_cmplx(FB * Fx, sf), dim=-1, keepdim=False) / (sf ** 2)
    ax = ax.repeat(1, 1, sf, sf)
    #aaT = torch.sum(splits_cmplx(F2B, sf), dim=-1, keepdim=False) / (sf ** 2)
    #aaT = aaT.repeat(1, 1, sf, sf)
    Fx += (FBFy - FBC * ax) / (aaT + rho*(sf**2))
    Xproj = torch.fft.irfftn(Fx, Fx.shape[-2:], dim=(-2, -1))
    return Xproj

def project_operator_cmplx(x, FB, FBC, aaT, FBFy, sf):
    Fx = torch.fft.fftn(x, dim=(-2, -1))
    ax = torch.sum(splits_cmplx(FB*Fx, sf), dim=-1, keepdim=False) / (sf**2)
    ax = ax.repeat(1, 1, sf, sf)
    #aaT = torch.sum(splits_cmplx(F2B, sf), dim=-1, keepdim=False) / (sf**2)
...
def downsample_fourier(x, FB, sf):
    Fx = torch.fft.fftn(x, dim=(-2, -1))
    ax = torch.sum(splits_cmplx(FB * Fx, sf), dim=-1, keepdim=False) / (sf**2)
    Xdwn = torch.fft.irfftn(ax, ax.shape[-2:], dim=(-2, -1))
    return Xdwn

def calculate_lambda(aaT, FB, rho, sf):
    a_abs = torch.abs(FB) / sf
    #max_noise_id = torch.argmax(a_abs / (aaT + rho)).item()
    #aaT_m = aaT.view(-1)[max_noise_id]
    #a_abs_m = a_abs.view(-1)[max_noise_id]
    #lambda_reg = (a_abs_m**2)/(4*aaT_m)

    lambda_reg = torch.max((a_abs**2)/(4*aaT))
    return lambda_reg.item()


def splits_cmplx(a, sf):
    '''split a into sfxsf distinct blocks
...
def unsplits_cmplx(a, sf):
    ...
    Returns:
        b: NxCx(W/sf)x(H/sf)x(sf^2)
    '''
    b = torch.cat(torch.chunk(a, sf, dim=4), dim=3)
    b = torch.cat(torch.chunk(b, sf, dim=4), dim=2)
    return b
...
def classical_degradation(x, k, sf=3, mode='wrap'):
    ''' blur + downsampling
    ...
    Return:
        downsampled LR image
    '''
    x = ndimage.filters.convolve(x, np.expand_dims(k, axis=2), mode=mode)
