...
class UNetRes_div(nn.Module):
    # divise par la moyenne des carres des pixels + epsilon 
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose', epsilon = 1e-5, bias = False):
        super(UNetRes_div, self).__init__()
    ...
    def forward(self, x0):
        ...
        return x
class UNetRes_inv(nn.Module):
    # inverse l'input 
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose', epsilon = 1e-5, bias = False):
        super(UNetRes_inv, self).__init__()
        self.m_head = B.conv(in_nc, nc[0], bias=bias, mode='C')
        self.epsilon = epsilon
        # downsample
        if downsample_mode == 'avgpool':
...
class UNetRes_bias(nn.Module):
    ...
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose', bias = False):
        super(UNetRes_bias, self).__init__()
        self.m_head = B.conv(in_nc, nc[0], bias=bias, mode='C')
